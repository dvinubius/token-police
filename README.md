...
- **Conversations list** — every conversation sorted by last activity, with a
  source badge (`CC` / `Codex`), project, title, cost, tokens, human request
...
- **Conversation detail** — click any conversation to see its totals plus a table
  ...
  cache write, LLM call count, and cost.
...
| `GET /api/conversations/:id/turns` | Every LLM call for one conversation, including its parent human request. |
...
Costs use the `model` recorded in each LLM call, falling back to
