...
| Sessions | `#sessions` or Session selection | Filter Sessions and inspect selected Session detail. | Filter controls, Session list, detail pane with Initial session prompt, sortable Human request table. | Filter, clear filters, select Session, sort Human requests, open Human request dialog. |
...
| LLM Call Dialog | Human request row activation | Inspect calls for a Human request. | Request prompt, model summary, request totals, sortable fixed-layout LLM call table, expandable call insights. | Sort calls, expand or collapse calls, close dialog, scan high-cost calls. |
