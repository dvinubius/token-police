...
| Sessions | `#sessions` or Session selection | Filter Sessions and inspect selected Session detail. | Filter controls, Session list, detail pane, sortable Human request table. | Filter, clear filters, select Session, sort Human requests, open Human request dialog. |
...
| LLM Call Dialog | Human request row activation | Inspect calls for a Human request. | Full Human request, request totals, sortable LLM call table. | Sort calls, close dialog, scan high-cost calls. |
